...
const CourseDetails = () => {
    ...
    const fetchCourseDetailsHandler = async (courseId) => { // Renamed to avoid conflict
        setLoading(true);
        try {
            const result = await fetchCourseDetails(courseId); // Updated function call
            if (!result) {
    ...
    useEffect(() => {
        fetchCourseDetailsHandler(courseId); // Updated function call
    }, [courseId]);
    ...
    const isOpen = (index) => {
        return openSectionsArray.includes(index);
    };
                ...
                <div className='w-[65%] text-white flex flex-col gap-7'>
                    ...
                    <div className='flex flex-col gap-3'>
                        ...
                        <div className='flex justify-between items-center'>
                            <p>{`${course?.courseContent?.length} section(s) ${(course?.courseContent?.flatMap((section) => section.subSection))?.length} lecture(s) 10s total length`}</p>
                            <div className='text-yellow-50 cursor-pointer' onClick={() => setOpenSectionsArray([])}>Collapse all sections</div>
                        </div>
                        <div>
                            {course?.courseContent?.map((section, index) => (
                                <details key={index} open={isOpen(index)} onClick={() => setOpenSectionsArray(prevSections => prevSections.includes(index) ? prevSections.filter(i => i !== index) : [...prevSections, index])}>
